...
#define FOREACH_ENUM_ELOCKTYPES(op) \
	op(ELockTypes::VE_None) \
	op(ELockTypes::VE_Key) \
	op(ELockTypes::VE_Number) \
	op(ELockTypes::VE_DeadBolt) \
	op(ELockTypes::VE_Finger) 

enum class ELockTypes;
template<> struct TIsUEnumClass<ELockTypes> { enum { Value = true }; };
